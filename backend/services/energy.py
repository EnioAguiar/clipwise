...
Adapted from podcli/backend/services/audio_analyzer.py for ClipWise.
"""
...
def _run_cmd(cmd: list, timeout: float = 600) -> subprocess.CompletedProcess:
...
def extract_audio_energy(video_path: str, window_sec: float = 1.0) -> list[dict]:
    ...
    Returns list of {time, rms_db} dicts, one per window.
    Uses FFmpeg astats filter on audio only (-vn skips video decoding).
    """
    cmd = [
        "ffmpeg",
        "-i", video_path,
...
def _fallback_energy(video_path: str) -> list[dict]:
    ...
    probe = _run_cmd(probe_cmd, timeout=30)
    try:
...
def compute_energy_scores(energy_data: list[dict], segments: list[dict]) -> list[float]:
    ...
    Uses z-score normalization so it adapts to each video's baseline.
    Formula: z_avg * 0.4 + z_peak * 0.6 (peak weighted higher)
    """
    if not energy_data:
...
def get_energy_profile(video_path: str, segments: list[dict] = None) -> dict:
    ...
    # Compute mean_rms from valid energy data
    valid_rms = [e["rms_db"] for e in energy_data if e["rms_db"] > -60]
    mean_rms = sum(valid_rms) / len(valid_rms) if valid_rms else -60.0
...
def save_energy_data(
    ...
    """
    Save energy analysis results to /tmp/clipwise/<video_id>/energy.json.
    ...
    energy_path = os.path.join(dest_dir, "energy.json")
